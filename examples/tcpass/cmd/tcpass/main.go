...
func main() {
	objPath := flag.String("obj", "dist/tc.bpf.o", "compiled Horizon eBPF object")
	ifaceName := flag.String("iface", "", "network interface to attach the TC program to")
...
func run(objPath string, ifaceName string, timeout time.Duration) error {
	...
	defer func() {
		if objs.PassIngress != nil {
			_ = objs.PassIngress.Close()
		}
	}()

	l, err := link.AttachTCX(link.TCXOptions{
		Program:   objs.PassIngress,
	...
	fmt.Printf("TC ingress pass program attached on %s; press Ctrl-C to detach\n", ifaceName)
	<-ctx.Done()
	return nil
}
