...
_Static_assert(sizeof(struct hzn_type_ExecEvent) == 32, "horizon: struct ExecEvent size mismatch");
...
static __always_inline struct hzn_type_ExecEvent *Events_reserve(void) {
    return bpf_ringbuf_reserve(&Events, sizeof(struct hzn_type_ExecEvent), 0);
}

static __always_inline void Events_submit(struct hzn_type_ExecEvent *value) {
    bpf_ringbuf_submit(value, 0);
}

static __always_inline void Events_discard(struct hzn_type_ExecEvent *value) {
    bpf_ringbuf_discard(value, 0);
}

static __always_inline bool hzn_fn_shouldEmit(struct hzn_type_ExecEvent *e, __u32 pid);
...
int OnExec(struct trace_event_raw_sched_process_exec *ctx) {
    (void)ctx;
    struct hzn_type_ExecEvent *e = Events_reserve();
    if (e == 0) {
        return 0;
    }
    e->ts_ns = hzn_ktime_get_ns();
    e->pid = hzn_current_pid();
    hzn_current_comm(&e->comm, sizeof(e->comm));
    bool emit = hzn_fn_shouldEmit(e, e->pid);
    if (!emit) {
        Events_discard(e);
        return 0;
    }
