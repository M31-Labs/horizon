// Code generated by hzn; DO NOT EDIT.

package bindings

import (
	"bytes"
	"context"
	"encoding/binary"
	"encoding/json"
	"errors"
	"fmt"
	"unsafe"

	"github.com/cilium/ebpf"
	"github.com/cilium/ebpf/link"
	"github.com/cilium/ebpf/ringbuf"
	"github.com/cilium/ebpf/rlimit"
	"m31labs.dev/horizon/capability"
)

const CapabilityManifestJSON = "{\n  \"schema\": \"m31labs.dev/horizon/capability/v1\",\n  \"package\": \"mfbt\",\n  \"programs\": [\n    {\n      \"name\": \"OnExec\",\n      \"kind\": \"tracepoint\",\n      \"attach\": \"sched:sched_process_exec\",\n      \"section\": \"tracepoint/sched:sched_process_exec\",\n      \"capabilities\": [\n        \"kernel.process.exec.observe\"\n      ]\n    }\n  ],\n  \"capabilities\": [\n    {\n      \"name\": \"kernel.process.exec.observe\",\n      \"kind\": \"source\",\n      \"danger\": {\n        \"mode\": \"observe\",\n        \"scope\": \"event\",\n        \"reversibility\": \"none\"\n      },\n      \"program\": \"OnExec\",\n      \"section\": \"tracepoint/sched:sched_process_exec\",\n      \"emits\": \"LinuxExecEvent\",\n      \"maps\": {\n        \"read\": [],\n        \"write\": [],\n        \"events\": [\n          \"Events\"\n        ]\n      },\n      \"requirements\": {\n        \"min_kernel\": \"5.8\",\n        \"programs\": [\n          {\n            \"name\": \"tracepoint\",\n            \"min_kernel\": \"4.7\"\n          }\n        ],\n        \"maps\": [\n          {\n            \"name\": \"ringbuf\",\n            \"min_kernel\": \"5.8\"\n          }\n        ],\n        \"helpers\": [\n          {\n            \"name\": \"bpf_get_current_comm\",\n            \"min_kernel\": \"4.1\"\n          },\n          {\n            \"name\": \"bpf_get_current_pid_tgid\",\n            \"min_kernel\": \"4.1\"\n          },\n          {\n            \"name\": \"bpf_ktime_get_ns\",\n            \"min_kernel\": \"4.1\"\n          },\n          {\n            \"name\": \"bpf_ringbuf_reserve\",\n            \"min_kernel\": \"5.8\"\n          },\n          {\n            \"name\": \"bpf_ringbuf_submit\",\n            \"min_kernel\": \"5.8\"\n          }\n        ],\n        \"permissions\": [\n          \"bpf_program_load\",\n          \"perf_event_open\"\n        ],\n        \"features\": [\n          \"tracefs\"\n        ]\n      },\n      \"helper_effects\": [\n        {\n          \"name\": \"bpf.current_comm\",\n          \"observes\": [\n            \"task.comm\"\n          ]\n        },\n        {\n          \"name\": \"bpf.current_pid\",\n          \"observes\": [\n            \"task.tgid\"\n          ]\n        },\n        {\n          \"name\": \"bpf.ktime_get_ns\",\n          \"observes\": [\n            \"kernel.time.monotonic\"\n          ]\n        },\n        {\n          \"name\": \"ringbuf.reserve\",\n          \"mutates\": [\n            \"ringbuf:Events\"\n          ],\n          \"resource\": \"reserve\"\n        },\n        {\n          \"name\": \"ringbuf.submit\",\n          \"mutates\": [\n            \"ringbuf:Events\"\n          ],\n          \"resource\": \"submit\"\n        }\n      ]\n    }\n  ],\n  \"maps\": [\n    {\n      \"name\": \"Events\",\n      \"kind\": \"ringbuf\",\n      \"value\": \"LinuxExecEvent\"\n    }\n  ],\n  \"types\": [\n    {\n      \"name\": \"LinuxExecEvent\",\n      \"kind\": \"struct\",\n      \"size\": 32,\n      \"align\": 8,\n      \"fields\": [\n        {\n          \"name\": \"ts_ns\",\n          \"type\": \"u64\",\n          \"offset\": 0\n        },\n        {\n          \"name\": \"pid\",\n          \"type\": \"u32\",\n          \"offset\": 8\n        },\n        {\n          \"name\": \"comm\",\n          \"type\": \"[16]u8\",\n          \"offset\": 12\n        }\n      ]\n    }\n  ],\n  \"requirements\": {\n    \"min_kernel\": \"5.8\",\n    \"programs\": [\n      {\n        \"name\": \"tracepoint\",\n        \"min_kernel\": \"4.7\"\n      }\n    ],\n    \"maps\": [\n      {\n        \"name\": \"ringbuf\",\n        \"min_kernel\": \"5.8\"\n      }\n    ],\n    \"helpers\": [\n      {\n        \"name\": \"bpf_get_current_comm\",\n        \"min_kernel\": \"4.1\"\n      },\n      {\n        \"name\": \"bpf_get_current_pid_tgid\",\n        \"min_kernel\": \"4.1\"\n      },\n      {\n        \"name\": \"bpf_ktime_get_ns\",\n        \"min_kernel\": \"4.1\"\n      },\n      {\n        \"name\": \"bpf_ringbuf_reserve\",\n        \"min_kernel\": \"5.8\"\n      },\n      {\n        \"name\": \"bpf_ringbuf_submit\",\n        \"min_kernel\": \"5.8\"\n      }\n    ],\n    \"permissions\": [\n      \"bpf_program_load\",\n      \"perf_event_open\"\n    ],\n    \"features\": [\n      \"tracefs\"\n    ]\n  }\n}\n"

func CapabilityManifest() (capability.Manifest, error) {
	var manifest capability.Manifest
	if err := json.Unmarshal([]byte(CapabilityManifestJSON), &manifest); err != nil {
		return capability.Manifest{}, err
	}
	return manifest, nil
}

type LinuxExecEvent struct {
	TsNs uint64
	Pid  uint32
	Comm [16]uint8
}

var _ [32 - int(unsafe.Sizeof(LinuxExecEvent{}))]byte
var _ [int(unsafe.Sizeof(LinuxExecEvent{})) - 32]byte
var _ [-int(unsafe.Offsetof(LinuxExecEvent{}.TsNs))]byte
var _ [8 - int(unsafe.Offsetof(LinuxExecEvent{}.Pid))]byte
var _ [int(unsafe.Offsetof(LinuxExecEvent{}.Pid)) - 8]byte
var _ [12 - int(unsafe.Offsetof(LinuxExecEvent{}.Comm))]byte
var _ [int(unsafe.Offsetof(LinuxExecEvent{}.Comm)) - 12]byte

type Objects struct {
	collection *ebpf.Collection
	Events     *ebpf.Map     `ebpf:"Events"`
	OnExec     *ebpf.Program `ebpf:"OnExec"`
}

type LoadOptions struct {
	Collection    *ebpf.CollectionOptions
	RemoveMemlock bool
	Programs      []string
}

func LoadObjects(path string) (*Objects, error) {
	return LoadObjectsWithOptions(path, LoadOptions{RemoveMemlock: true})
}

func LoadObjectsWithOptions(path string, opts LoadOptions) (*Objects, error) {
	if opts.RemoveMemlock {
		if err := rlimit.RemoveMemlock(); err != nil {
			return nil, fmt.Errorf("remove memlock limit: %w", err)
		}
	}
	spec, err := ebpf.LoadCollectionSpec(path)
	if err != nil {
		return nil, fmt.Errorf("load %s: %w", path, err)
	}
	if len(opts.Programs) != 0 {
		wanted := make(map[string]bool, len(opts.Programs))
		for _, name := range opts.Programs {
			if spec.Programs[name] == nil {
				return nil, fmt.Errorf("select eBPF program %q: not found", name)
			}
			wanted[name] = true
		}
		for name := range spec.Programs {
			if !wanted[name] {
				delete(spec.Programs, name)
			}
		}
		collectionOptions := ebpf.CollectionOptions{}
		if opts.Collection != nil {
			collectionOptions = *opts.Collection
		}
		collection, err := ebpf.NewCollectionWithOptions(spec, collectionOptions)
		if err != nil {
			return nil, fmt.Errorf("load selected eBPF objects: %w", err)
		}
		objects := &Objects{collection: collection}
		objects.Events = collection.Maps["Events"]
		objects.OnExec = collection.Programs["OnExec"]
		return objects, nil
	}
	var objects Objects
	if err := spec.LoadAndAssign(&objects, opts.Collection); err != nil {
		return nil, fmt.Errorf("load eBPF objects: %w", err)
	}
	return &objects, nil
}

func (o *Objects) Close() error {
	if o == nil {
		return nil
	}
	if o.collection != nil {
		o.collection.Close()
		return nil
	}
	var err error
	if o.Events != nil {
		err = errors.Join(err, o.Events.Close())
	}
	if o.OnExec != nil {
		err = errors.Join(err, o.OnExec.Close())
	}
	return err
}

func (o *Objects) ReadEvents(ctx context.Context, handle func(LinuxExecEvent) error) error {
	if o == nil || o.Events == nil {
		return fmt.Errorf("Events map is not loaded")
	}
	if ctx == nil {
		ctx = context.Background()
	}
	reader, err := ringbuf.NewReader(o.Events)
	if err != nil {
		return err
	}
	defer reader.Close()
	done := make(chan struct{})
	defer close(done)
	go func() {
		select {
		case <-ctx.Done():
			_ = reader.Close()
		case <-done:
		}
	}()
	for {
		record, err := reader.Read()
		if err != nil {
			if errors.Is(err, ringbuf.ErrClosed) && ctx.Err() != nil {
				return ctx.Err()
			}
			return err
		}
		var event LinuxExecEvent
		if err := binary.Read(bytes.NewReader(record.RawSample), binary.LittleEndian, &event); err != nil {
			return err
		}
		if err := handle(event); err != nil {
			return err
		}
	}
}

func (o *Objects) AttachOnExec() (link.Link, error) {
	if o == nil || o.OnExec == nil {
		return nil, fmt.Errorf("OnExec program is not loaded")
	}
	return link.Tracepoint("sched", "sched_process_exec", o.OnExec, nil)
}
