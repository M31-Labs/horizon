...
_Static_assert(__builtin_offsetof(struct hzn_type_ExecEvent, uid) == 12, "horizon: struct ExecEvent.uid offset mismatch");
_Static_assert(__builtin_offsetof(struct hzn_type_ExecEvent, comm) == 16, "horizon: struct ExecEvent.comm offset mismatch");
...
int OnExec(struct trace_event_raw_sched_process_exec *ctx) {
    (void)ctx;
    struct hzn_type_ExecEvent *event = Events_reserve();
    if (event == 0) {
        return 0;
    }
