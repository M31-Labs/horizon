...
// For functions with typed statements, it consumes sites.Loops directly.
// For functions with only raw/text bodies (legacy path), it falls back to
// the text-based scan in ValidateLoops; those functions are not present in
// Sites because Collect skips them.
func AnalyzeLoops(program ir.Program, sites Sites) []diag.Diagnostic {
	loopBounds := integerLoopBounds(program.Constants)

	// Typed path: check each collected loop site directly — no re-walk.
	var diags []diag.Diagnostic
	for _, site := range sites.Loops {
		stmt := site.Stmt
		switch {
